...
qPS_Vs_22qNOPS <- patho_df %>%
  ...
  mutate(PS = ifelse(PS == 0, "22q11DS-", "22q11DS+"))
...
#print plot
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/22q+_vs_22q-_barplot.pdf',10,10)
qPS_Vs_22qNOPS
dev.off()
```
...
# iterate average dataframe by feature and display as boxplot
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/emotrics_barplot.pdf')
for (r in 1:length(rois)){
  roi = rois[r]
  roi_data <- roi_df[, c('group', 'bbl_id', roi)] %>%
    group_by(group) %>%
    summarise(mean = mean(get(roi)), sd = sd(get(roi)))
...
plt_list <- list()
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/22qPS_barplot.pdf')
for (r in 1:length(rois)){
...
#plot all ROC
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/ROC_PS_TD-loo.pdf')
